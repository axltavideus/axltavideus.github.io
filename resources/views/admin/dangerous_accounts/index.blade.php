...
<div class="container py-5">
    ...
    <div class="mb-4">
        <a href="{{ route('admin.dangerous_accounts.create') }}" class="btn btn-warning mb-3">Add New Dangerous Account</a>

        <form method="GET" action="{{ route('admin.dangerous_accounts.index') }}" class="d-flex align-items-center gap-2 mb-3">
                ...
                <tr>
                    ...
                    <td>
                        @if($account->bukti_file_path)
                            <a href="{{ asset('storage/' . $account->bukti_file_path) }}" target="_blank" class="text-warning">View Evidence</a>
                        @else
                            None
                        @endif
                    </td>
                    ...
                    <td>
                        <a href="{{ route('admin.dangerous_accounts.edit', $account->id) }}" class="btn btn-warning btn-sm mb-1">
                            <i class="fas fa-edit" style="color: white;"></i> <!-- Edit Icon -->
                        </a>
                        <form action="{{ route('admin.dangerous_accounts.destroy', $account->id) }}" method="POST" style="display:inline-block;" onsubmit="return confirm('Are you sure you want to delete this dangerous account?');">
                            @csrf
                            @method('DELETE')
